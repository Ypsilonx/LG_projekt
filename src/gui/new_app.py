...
class ClimateApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.scheduler_widget = SchedulerWidget(
            self.scrollable_frame,
            on_schedule_execute=self.execute_scheduled_command
        )
    ...
    def handle_device_command(self, command, *args):
        ...
        def handle_result():
            try:
                ...
                self.after(0, lambda: messagebox.showerror("Chyba", f"Příkaz {command} selhal: {e}"))
    ...
    async def _execute_device_command(self, command, *args):
        ...
        try:
            ...
            if command == "toggle_power":
                ...
                current_state = status.get("runState", {}).get("currentState", "POWER_OFF")
                new_state = "POWER_ON" if current_state == "POWER_OFF" else "POWER_OFF"
    ...
    def _update_gui_status(self, device_status):
        ...
        try:
            ...
            power_state = device_status.get("runState", {}).get("currentState", "UNKNOWN")
            mode = device_status.get("airConJobMode", {}).get("currentJobMode", "N/A")
            temp = device_status.get("temperature", {}).get("currentTemperature", "?")
            
            status_text = f"Stav: {power_state}, Režim: {mode}, Teplota: {temp}°C"
            self.status_var.set(status_text)
            
            # LED indikátor
            if power_state == "POWER_ON":
                self.led_indicator.set_state("on")
            elif power_state == "POWER_OFF":
                self.led_indicator.set_state("off")
            else:
